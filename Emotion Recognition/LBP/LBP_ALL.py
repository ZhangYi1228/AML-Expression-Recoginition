...
def uniform_LBP(src, norm=True):
    ...
    :param norm:Whether to normalize to the gray space of [0-255]
    :return:
    '''
    ...
    for x in range(radius, width - radius - 1):
        for y in range(radius, height - radius - 1):
            ...
            for n in range(n_points):
                ...
                # Map the coordinates between 0-1
                tx = np.abs(x - x1)
                ty = np.abs(y - y1)

                # Calculate the weight according to the weight calculation formula of x, y between 0-1
                w1 = (1 - tx) * (1 - ty)
                w2 = tx * (1 - ty)
                w3 = (1 - tx) * ty
                w4 = tx * ty

                # Calculate the gray value of the kth sampling point according to the bilinear interpolation formula
...
def disp_test_result(img, gray, dst, mode=0):
    '''
    :param mode:0,Opencv displays pictures; 1, matplotlib displays pictures.
    :return:
    '''
    if mode == 0:
        cv2.imshow('src', img)
